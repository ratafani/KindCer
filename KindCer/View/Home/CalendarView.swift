...
struct DateView:View {
    ...
    //            }
                ...
                HStack {
                    ...
                    DayView(day:"Sel", date: $arrday[1],isPicked: $arrPicked[1]).padding(.horizontal,4).onTapGesture {
                        //                self.mDate = self.arrDate[1]
                        //                print("tap")
                        self.taped(index: 1)
                    }
                    DayView(day:"Rab", date: $arrday[2],isPicked: $arrPicked[2]).padding(.horizontal,4).onTapGesture {
                        //                self.mDate = self.arrDate[2]
                        //                print("tap")
                        self.taped(index: 2)
                    }
                    DayView(day:"Kam", date: $arrday[3],isPicked: $arrPicked[3]).padding(.horizontal,4).onTapGesture {
                        //                self.mDate = self.arrDate[3]
                        self.taped(index: 3)
                    }
                    DayView(day:"Jum", date: $arrday[4],isPicked: $arrPicked[4]).padding(.horizontal,4).onTapGesture {
                        //                self.mDate = self.arrDate[4]
                        self.taped(index: 4)
                    }
                    DayView(day:"Sab", date: $arrday[5],isPicked: $arrPicked[5]).padding(.horizontal,4).onTapGesture {
                        //                self.mDate = self.arrDate[5]
                        self.taped(index: 5)
                    }
                    DayView(day:"Min", date: $arrday[6],isPicked: $arrPicked[6]).padding(.horizontal,4).onTapGesture {
        ...
        return VStack {
            ...
            ZStack {
                ...
                Text("\(date)").font(.system(size: 20, design: .default)).foregroundColor(isPicked ? Color.white:Color.black).padding(.bottom,15)
                    .background(Circle().foregroundColor(isPicked ? Color("Primary"): Color.white).frame(width: 30, height: 30).offset(x: 0, y: -7))
            }
